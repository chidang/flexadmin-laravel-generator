...
use Techamz\Generator\Utils\TableFieldsGenerator;
...
class ModelGenerator extends BaseGenerator
{
    ...
    {
        $templateData = view('flex-laravel-generator::model.model', $this->variables())->render();

        g_filesystem()->createFile($this->path.$this->fileName, $templateData);

        $this->config->commandComment(techamz_nl().'Model created: ');
    ...
    {
        return [
            'fillables'        => implode(','.techamz_nl_tab(1, 2), $this->generateFillables()),
            'casts'            => implode(','.techamz_nl_tab(1, 2), $this->generateCasts()),
            'rules'            => implode(','.techamz_nl_tab(1, 2), $this->generateRules()),
            ...
            'timestamps'       => config('flex_laravel_generator.timestamps.enabled', true),
        ];
    }
    ...
    {
        $deletedAt = config('flex_laravel_generator.timestamps.deleted_at', 'deleted_at');
    ...
    {
        $createdAt = config('flex_laravel_generator.timestamps.created_at', 'created_at');
    ...
    {
        $updatedAt = config('flex_laravel_generator.timestamps.updated_at', 'updated_at');
    ...
    public function generateSwagger(): string
    {
        ...
        return view('swagger-generator::model.model', [
            ...
            'properties'     => implode(','.techamz_nl().' ', $properties),
        ]);
    }
    ...
    {
        $dont_require_fields = config('flex_laravel_generator.options.hidden_fields', [])
                + config('flex_laravel_generator.options.excluded_fields', $this->excluded_fields);
    ...
    protected function generateRelations(): string
    {
        ...
        return implode(techamz_nl_tab(2), $relations);
    }
