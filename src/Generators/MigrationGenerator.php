...
class MigrationGenerator extends BaseGenerator
{
    public function __construct()
    {
        ...
        $this->path = config('flex_laravel_generator.path.migration', database_path('migrations/'));
    }

    public function generate()
    {
        $templateData = view('flex-laravel-generator::migration', $this->variables())->render();
        ...
        $this->config->commandComment(techamz_nl().'Migration created: ');
    ...
    protected function generateFields(): string
    {
        ...
        if ($this->config->options->softDelete) {
            $softDeleteFieldName = config('flex_laravel_generator.timestamps.deleted_at', 'deleted_at');
        ...
        return implode(techamz_nl_tab(1, 3), array_merge($fields, $foreignKeys));
    }
