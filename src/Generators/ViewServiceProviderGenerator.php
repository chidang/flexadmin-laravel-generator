...
class ViewServiceProviderGenerator extends BaseGenerator
{
    ...
    {
        $templateData = get_template_file_path('view_service_provider', 'flex-laravel-generator');
    ...
    public function addViewComposer(): string
    {
        ...
        $newViewStatement = get_template('scaffold.view_composer', 'flex-laravel-generator');
        $newViewStatement = fill_template($this->config->dynamicVars, $newViewStatement);

        $newViewStatement = techamz_nl().$newViewStatement;
    ...
    public function addCustomProvider()
    {
        ...
        if ($customProviders && !$isExist) {
            $newChanges = substr_replace(
                $file,
                techamz_nl().techamz_tab(8).'\App\Providers\ViewServiceProvider::class,',
    ...
    public function addNamespace($model, $mainViewContent)
    {
        ...
        $newModelStatement = techamz_nl().$newModelStatement;
        if (!$isNameSpaceExist) {
