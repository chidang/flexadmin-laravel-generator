...
use Techamz\Generator\Common\GeneratorField;
use Techamz\Generator\Common\GeneratorFieldRelation;
use Techamz\Generator\Events\GeneratorFileCreated;
use Techamz\Generator\Events\GeneratorFileCreating;
use Techamz\Generator\Events\GeneratorFileDeleted;
use Techamz\Generator\Events\GeneratorFileDeleting;
use Techamz\Generator\Generators\API\APIControllerGenerator;
use Techamz\Generator\Generators\API\APIRequestGenerator;
use Techamz\Generator\Generators\API\APIResourceGenerator;
...
use Techamz\Generator\Utils\GeneratorFieldsInputUtil;
...
class BaseCommand extends Command
{
        ...
        if ($runMigration) {
            if ($this->option('forceMigrate')) {
                ...
                if ($this->option('jsonFromGUI') && $requestFromConsole) {
                    $this->runMigration();
                } elseif ($requestFromConsole && $this->confirm(techamz_nl().'Do you want to migrate database? [y|N]', false)) {
                    $this->runMigration();
                }
            }
        }
    ...
    {
        $migrationPath = config('flex_laravel_generator.path.migration', database_path('migrations/'));
    ...
    protected function saveSchemaFile()
    {
        ...
        $path = config('flex_laravel_generator.path.schema_files', resource_path('model_schemas/'));
    ...
    protected function getFieldsFromConsole()
    {
        ...
        if (config('flex_laravel_generator.timestamps.enabled', true)) {
            $this->addTimestamps();
        }
    }
    ...
    protected function parseFieldsFromJsonFile()
    {
        ...
        if (file_exists($fieldsFileValue)) {
            ...
            $schemaFileDirector = config(
                'flex_laravel_generator.path.schema_files',
