...
namespace Techamz\Generator\Commands\Publish;
...
class PublishUserCommand extends PublishBaseCommand
{
    ...
    public function handle()
    {
        ...
        if (config('flex_laravel_generator.options.repository_pattern')) {
    ...
    {
        $viewsPath = config('flex_laravel_generator.path.views', resource_path('views/'));
        $templateType = config('flex_laravel_generator.templates', 'flexadmin-templates');
    ...
    {
        $path = config('flex_laravel_generator.path.routes', base_path('routes/web.php'));

        $routeContents = g_filesystem()->getFile($path);
        $controllerNamespace = config('flex_laravel_generator.namespace.controller');
        $routeContents .= techamz_nls(2)."Route::resource('users', '$controllerNamespace\UserController')->middleware('auth');";
    ...
    private function updateMenu()
    {
        ...
        $menuContents .= techamz_nl().$usersMenuContent;
    ...
    private function publishUserController()
    {
        ...
        $controllerPath = config('flex_laravel_generator.path.controller', app_path('Http/Controllers/'));
        $controllerPath .= 'UserController.php';

        $controllerContents = view('flex-laravel-generator::scaffold.user.'.$name)->render();
    ...
    {
        $repositoryPath = config('flex_laravel_generator.path.repository', app_path('Repositories/'));
        ...
        $templateData = view('flex-laravel-generator::scaffold.user.user_repository')->render();
    ...
    {
        $requestPath = config('flex_laravel_generator.path.request', app_path('Http/Requests/'));
        ...
        $templateData = view('flex-laravel-generator::scaffold.user.create_user_request')->render();
    ...
    private function publishUpdateUserRequest()
    {
        ...
        $templateData = view('flex-laravel-generator::scaffold.user.update_user_request')->render();
        g_filesystem()->createFile($requestPath.$fileName, $templateData);
