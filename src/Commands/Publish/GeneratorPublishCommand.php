...
class GeneratorPublishCommand extends PublishBaseCommand
{
    ...
    public function handle()
    {
        ...
        $repositoryPattern = config('flex_laravel_generator.options.repository_pattern', true);
    ...
    private function updateRouteServiceProvider()
    {
        ...
        $search = "Route::middleware('api')".techamz_nl().str(' ')->repeat(16)."->prefix('api')";
        $beforeContent = str($fileContent)->before($search);
        $afterContent = str($fileContent)->after($search);

        $finalContent = $beforeContent.$search.techamz_nl().str(' ')->repeat(16)."->as('api.')".$afterContent;
        g_filesystem()->createFile($routeServiceProviderPath, $finalContent);
    }

    private function publishTestCases()
    {
        $testsPath = config('flex_laravel_generator.path.tests', base_path('tests/'));
        $testsNameSpace = config('flex_laravel_generator.namespace.tests', 'Tests');
        $createdAtField = config('flex_laravel_generator.timestamps.created_at', 'created_at');
        $updatedAtField = config('flex_laravel_generator.timestamps.updated_at', 'updated_at');

        $templateData = view('flex-laravel-generator::api.test.api_test_trait', [
        ...
        $testAPIsPath = config('flex_laravel_generator.path.api_test', base_path('tests/APIs/'));
        if (!file_exists($testAPIsPath)) {
            g_filesystem()->createDirectoryIfNotExist($testAPIsPath);
            $this->info('APIs Tests directory created');
        }

        $testRepositoriesPath = config('flex_laravel_generator.path.repository_test', base_path('tests/Repositories/'));
    ...
    private function publishBaseController()
    {
        ...
        $templateData = view('flex-laravel-generator::stubs.app_base_controller', [
            'namespaceApp' => $this->getLaravel()->getNamespace(),
            'apiPrefix'    => config('flex_laravel_generator.api_prefix'),
        ])->render();
    ...
    private function publishBaseRepository()
    {
        ...
        $templateData = view('flex-laravel-generator::stubs.base_repository', [
            'namespaceApp' => $this->getLaravel()->getNamespace(),
        ])->render();
